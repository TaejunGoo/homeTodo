...
homeTodo는 공유 Space에서 반복 할 일을 생성·수정·완료하고 이력을 확인하는 학습용 MVP까지 구현한 뒤 신규 개발을 종료했다.

완료 이력 화면에서는 완료/완료 취소 이벤트를 날짜별로 조회하고 완료자와 시간을 표시한다.
오늘 화면은 반복 주기별 섹션, 현재 기간, 최근 3개 기간의 완료 상태를 보여 준다.
하단 탭은 제거하고 공통 헤더에서 Space 선택과 설정에 진입하도록 정리했다.
전체 TODO 관리 화면은 MVP에서 제외하고, TODO 수정·삭제와 개별 이력은 홈의 각 TODO 메뉴에서 진입한다.
생성·수정·삭제·완료·완료 취소 흐름은 두 계정으로 공유 Space에서 확인했다.
사용자에게 보이는 앱 문구는 범용성을 위해 `집안일`보다 `할 일`을 우선 사용한다.

Phase 13과 Phase 14의 미완료 체크 항목은 구현되지 않았거나 최종 검증하지 않은 상태를 그대로 나타낸다.
전체 구현 범위, 실행 방법, 미구현 기능은 루트 `README.md`에 최종 상태로 기록했다.

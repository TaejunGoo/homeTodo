...
- 상단 영역: 현재 화면 제목, 현재 Space 이름, 필요한 경우 설정 버튼
- 본문 영역: 목록, 빈 상태, 로딩 상태, 에러 상태
- 하단 영역: 주요 액션 버튼 또는 탭 내비게이션
- 기본 내비게이션: 로그인 이후 `오늘`, `전체 TODO`, `이력`, `설정` 탭
...
- Space 선택: 내가 속한 Space 목록, Space 만들기 버튼, 초대 코드 입력 화면으로 이동하는 버튼
- 오늘: 현재 기간 라벨, 미완료 집안일 목록, 완료된 집안일 목록, 완료 버튼

...
- 상단 헤더: 현재 Space 선택과 Space 생성 진입
- 본문 영역: Space 대시보드, 목록, 빈 상태, 로딩 상태, 에러 상태
- 우측 하단 플로팅 버튼: TODO 생성 진입
- 하단 고정 버튼 영역: 생성/수정/취소처럼 현재 화면의 주요 액션
- 기본 내비게이션: MVP에서는 Space 대시보드를 중심으로 시작하고, 필요하면 `전체 TODO`, `이력`, `설정` 탭을 분리한다.
...
- `IconButton`: Space 생성, 메뉴 열기, 닫기 같은 아이콘 액션
- `FloatingActionButton`: TODO 생성 화면으로 이동하는 우측 하단 플러스 버튼
- `AppHeader`: Space 선택 버튼, Space 이름, Space 생성 버튼을 포함하는 상단 헤더
...
- 우측: 플러스 아이콘 버튼을 표시한다.
- 우측 플러스 버튼을 누르면 Space 생성 화면으로 이동한다.
- 현재 Space가 없으면 Space 이름 대신 새 Space 생성을 유도하는 문구를 표시한다.
...
- 전체 TODO: 활성 TODO 목록, TODO 추가 버튼, TODO 수정/삭제 진입
- TODO 생성/수정: 제목 입력, 반복 타입 선택, 반복 값 입력, 저장 버튼, 삭제 버튼
- 완료 이력: 날짜 또는 기간별 완료 목록, 완료자 표시
- Space 설정: Space 이름, 멤버 목록, 초대 코드 생성/복사, 로그아웃
